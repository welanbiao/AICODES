...
public static class EnemyBuilder
{
    ...
    static GameObject Spirit(Transform parent, int elem)
    {
        ...
        var mat = Mats.Solid(c, Color.white, c * 0.45f, "sp" + elem);
        UnityEngine.Mesh body = MeshForge.Drop();
        if (elem == 0) body = MeshForge.Crystal();
        if (elem == 3) body = MeshForge.Flame();
        if (elem == 4) body = MeshForge.Golem();
        Danao.Mesh(root.transform, "body", body, Vector3.zero, new Vector3(0.7f, 0.95f, 0.7f), mat);
        Danao.Mesh(root.transform, "head", MeshForge.Sphere(20, 14), new Vector3(0, 0.48f, 0.05f), Vector3.one * 0.38f, mat);
        Danao.Mesh(root.transform, "core", MeshForge.Sphere(16, 12), new Vector3(0, 0.08f, 0.16f), Vector3.one * 0.18f,
            Mats.Solid(Color.white, c, c, "spc" + elem));
        Danao.Mesh(root.transform, "armL", MeshForge.Capsule(12, 6), new Vector3(-0.32f, 0.05f, 0), new Vector3(0.14f, 0.26f, 0.14f), mat);
        Danao.Mesh(root.transform, "armR", MeshForge.Capsule(12, 6), new Vector3(0.32f, 0.05f, 0), new Vector3(0.14f, 0.26f, 0.14f), mat);
        if (elem == 1)
            Danao.Mesh(root.transform, "leaf", MeshForge.Sphere(14, 10), new Vector3(0, 0.78f, 0), new Vector3(0.38f, 0.14f, 0.22f), mat);
        var bob = root.AddComponent<BobSpin>();
        bob.spin = new Vector3(0, 40 + elem * 8, 0);
        bob.amp = 0.14f;
        return root;
    }
